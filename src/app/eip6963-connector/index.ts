...
// listening 6963 event
const Providers:{[key:string]:any} = {};
(() => {
  if (typeof window === 'undefined') return;
  window.addEventListener('eip6963:announceProvider', (event:any) => {
    if (event.detail.info.uuid) {
      Providers[event.detail.info.name] = event.detail.provider
    }
  });
...
export class NoEthereumProviderError extends Error {
  public constructor() {
    ...
    this.message = 'No Ethereum provider was found on Providers.'
  }
}
...
export class EIP6963Connector extends AbstractConnector {
  provider: any;
  constructor(kwargs: AbstractConnectorArguments&{name:string}) {
    super(kwargs)
    ...
    this.provider = {};
    if(!Providers[kwargs.name] && (typeof window !== 'undefined')){
      this.provider = window.ethereum;
    }else{
      this.provider = Providers[kwargs.name]
    }
  }
